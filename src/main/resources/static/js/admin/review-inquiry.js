// /js/admin/review-inquiry.js

class ReviewInquiryManager {
    constructor() {
        this.currentPage = 0;
        this.pageSize = 20;
        this.currentStatus = '';
        this.currentSearch = '';
        this.targetType = 'REVIEW';
    ...
    async loadStats() {
        try {
            ...
            document.getElementById('totalCount').textContent = data.review || 0;
            // 리뷰 신고의 대기중 개수는 별도로 계산 필요 (현재 API에 없음)
            const reviewPending = await this.getReviewPendingCount();
            document.getElementById('pendingCount').textContent = reviewPending;
        } catch (error) {
    ...
    async getReviewPendingCount() {
        try {
            ...
            if (!response.ok) {
                throw new Error('대기중 개수 조회 실패');
            }

            const data = await response.json();
            return data.totalElements || 0;
        } catch (error) {
            console.error('대기중 개수 조회 실패:', error);
            return 0;
        }
    }
    ...
    renderTable(inquiries) {
        ...
        inquiries.forEach(inquiry => {
            ...
            row.innerHTML = `
                ...
                <td>${inquiry.email}</td>
                <td>${inquiry.targetTitle || '삭제된 리뷰'}</td>
                <td>${inquiry.subject}</td>
                <td><span class="status-badge ${inquiry.status.toLowerCase()}">${this.getStatusText(inquiry.status)}</span></td>
                <td><button class="btn btn-sm btn-primary" onclick="event.stopPropagation(); reviewInquiryManager.showDetail(${inquiry.id})">상세보기</button></td>
            `;
    ...
    renderDetailModal(inquiry) {
        ...
        modalBody.innerHTML = `
            ...
            <div class="detail-section">
                ...
                <div class="detail-info">
                    <div class="info-item">
                        ...
                        <span class="info-value">${inquiry.targetTitle || '삭제된 리뷰'}</span>
                    </div>
                    <div class="info-item">
            ...
            <div class="action-buttons">
                <button class="btn btn-success" onclick="reviewInquiryManager.updateStatus(${inquiry.id})">상태 변경</button>
                <button class="btn btn-secondary" onclick="reviewInquiryManager.closeModal()">닫기</button>
            </div>
        `;
    }
    ...
    async searchByEmail(email) {
        ...
        try {
            ...
            const data = await response.json();
            // 리뷰 신고만 필터링
            const reviewInquiries = data.content.filter(inquiry => inquiry.targetType === 'REVIEW');
            const filteredData = {
                ...data,
                content: reviewInquiries,
                totalElements: reviewInquiries.length
            };

            this.renderTable(filteredData.content);
            this.renderPagination(filteredData);
...
// 전역 변수로 인스턴스 생성
let reviewInquiryManager;

// DOM이 로드되면 초기화
document.addEventListener('DOMContentLoaded', function() {
    reviewInquiryManager = new ReviewInquiryManager();
});