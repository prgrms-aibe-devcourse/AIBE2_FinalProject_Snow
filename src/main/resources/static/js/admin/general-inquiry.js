// /js/admin/general-inquiry.js

class GeneralInquiryManager {
    constructor() {
        this.currentPage = 0;
        this.pageSize = 20;
        this.currentStatus = '';
    ...
    async loadStats() {
        try {
            ...
            document.getElementById('totalCount').textContent = data.general || 0;
            // 일반 신고의 대기중 개수는 별도로 계산 필요
            const generalPending = await this.getGeneralPendingCount();
            document.getElementById('pendingCount').textContent = generalPending;
        } catch (error) {
            ...
            <div class="detail-section">
                ...
                <div class="detail-info">
                    <div class="info-item">
                        ...
                        <span class="info-value">${inquiry.targetTitle || 'GENERAL'}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">제목</span>
                        <span class="info-value">${inquiry.subject || '일반 문의'}</span>
                    </div>
                </div>
            </div>
    ...
    async searchByEmail(email) {
        ...
        try {
            ...
            const data = await response.json();
            // 일반 신고만 필터링
            const generalInquiries = data.content.filter(inquiry => inquiry.targetType === 'GENERAL');
            const filteredData = {
                ...data,
                content: generalInquiries,
                totalElements: generalInquiries.length
            };
...
// 전역 변수로 인스턴스 생성
let generalInquiryManager;

// DOM이 로드되면 초기화
document.addEventListener('DOMContentLoaded', function() {
    generalInquiryManager = new GeneralInquiryManager();
});